<template>
    <main class="container-fluid d-flex align-items-center justify-content-center">
        <form class="container shadow rounded-5 p-4" v-on:submit.prevent="loginUser">
            <div class="row">
                <div class="col-12 pb-5">
                    <h2 class="text-center">Iniciar sesión</h2>
                    <span class="d-flex justify-content-center">
                        o:<router-link class="nav-link px-2" :to="{name:'signup'}">registrase</router-link>
                    </span>
                </div>
            </div>
            ...
            <div class="form-group my-4 d-flex justify-content-start">
                <button type="submit" class="btn btn-sm px-4 mx-2">iniciar</button>
...
<script setup>
    ...
    //funcion que permite loguear a un usuario a traves de la interfaz
    //args: void
    //return: void
    async function loginUser(){
        if(email.value && password.value){
            await store.startSession('login',email.value,password.value)
            if(store.token){
